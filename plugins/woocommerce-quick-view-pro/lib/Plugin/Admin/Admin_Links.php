...
namespace Barn2\WQV_Lib\Plugin\Admin;

use Barn2\WQV_Lib\Plugin\Plugin;
...
class Admin_Links implements Registerable, Service {
	...
	public function add_settings_link( $links ) {
		...
		array_unshift( $links, sprintf( '<a href="%1$s">%2$s</a>', esc_url( $settings_url ), __( 'Settings', 'woocommerce-quick-view-pro' ) ) );
	...
	public function add_documentation_link( $links, $file ) {
		...
		$row_meta = [
			'docs' => sprintf(
				...
				esc_attr( sprintf( __( 'View %s documentation', 'woocommerce-quick-view-pro' ), $this->plugin->get_name() ) ),
				esc_html__( 'Docs', 'woocommerce-quick-view-pro' )
			)
		];
