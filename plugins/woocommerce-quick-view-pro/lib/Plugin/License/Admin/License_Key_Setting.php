...
namespace Barn2\WQV_Lib\Plugin\License\Admin;

use Barn2\WQV_Lib\Admin\Settings_API_Helper;
use Barn2\WQV_Lib\Plugin\License\License;
...
class License_Key_Setting implements Registerable, License_Setting {
		...
		if ( $this->is_license_action( self::ACTIVATE_KEY ) ) {
			...
			if ( isset( $license_setting['license'] ) ) {
				...
				$this->add_settings_message(
					__( 'License key successfully activated.', 'woocommerce-quick-view-pro' ),
					__( 'There was an error activating your license key.', 'woocommerce-quick-view-pro' ),
					$activated
				);
			}
			...
			$this->add_settings_message(
				__( 'License key successfully deactivated.', 'woocommerce-quick-view-pro' ),
				__( 'There was an error deactivating your license key, please try again.', 'woocommerce-quick-view-pro' ),
				$deactivated
			);
			...
			$this->add_settings_message(
				__( 'The license key looks good!', 'woocommerce-quick-view-pro' ),
				__( 'There\'s a problem with your license key.', 'woocommerce-quick-view-pro' ),
	...
	public function get_license_key_setting() {
		...
		if ( $this->is_woocommerce ) {
			$setting['desc_tip'] = __( 'The licence key is contained in your order confirmation email.', 'woocommerce-quick-view-pro' );
		} elseif ( $this->is_edd ) {
			// EDD uses title case for setting names, so let's keep things consistent.
			$setting['title'] = __( 'License Key', 'woocommerce-quick-view-pro' );
	...
	private function get_license_description() {
		$buttons = [
			'check'      => $this->license_action_button( self::CHECK_KEY, __( 'Check', 'woocommerce-quick-view-pro' ) ),
			'activate'   => $this->license_action_button( self::ACTIVATE_KEY, __( 'Activate', 'woocommerce-quick-view-pro' ) ),
			'deactivate' => $this->license_action_button( self::DEACTIVATE_KEY, __( 'Deactivate', 'woocommerce-quick-view-pro' ) )
		];
		...
		if ( $this->license->is_active() ) {
			...
			if ( $this->license->is_inactive() && $this->is_license_action( 'deactivate_key' ) ) {
				...
				$message = __( 'License key deactivated.', 'woocommerce-quick-view-pro' );
			} else {
