...
use Barn2\Plugin\WC_Quick_View_Pro\Dependencies\Barn2\Setup_Wizard\Interfaces\Deferrable;
...
class Api implements JsonSerializable
{
    ...
    public function save_fields($request)
    {
        ...
        if (empty($step) || !$step instanceof Step) {
            return self::send_error_response(['message' => __('Could not find the appropriate step.', 'woocommerce-quick-view-pro')]);
        }
    ...
    public function handle_license($request)
    {
        ...
        if (empty($license_key)) {
            return self::send_error_response(['message' => __('Please enter a license key.', 'woocommerce-quick-view-pro')]);
        }
        if (!\in_array($action, $allowed_actions, \true)) {
            return self::send_error_response(['message' => __('Invalid action requested.', 'woocommerce-quick-view-pro')]);
        }
        $license_handler = $this->get_plugin()->get_license();
        switch ($action) {
            case 'activate':
