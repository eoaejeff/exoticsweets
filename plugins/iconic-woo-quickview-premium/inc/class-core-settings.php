...
if ( class_exists( 'Iconic_WQV_Core_Settings' ) ) {
	return;
}

/**
 * Iconic_WQV_Core_Settings.
 *
 * @class    Iconic_WQV_Core_Settings
 * @version  1.0.6
 */
class Iconic_WQV_Core_Settings {
	/**
	 * Single instance of the Iconic_WQV_Core_Settings object.
	 *
	 * @var Iconic_WQV_Core_Settings
	 */
	 ...
	 *
	 * @var Iconic_WQV_Settings_Framework
	 */
	...
	/**
	 * Creates/returns the single instance Iconic_WQV_Core_Settings object.
	 *
	 * @param array $args Arguments.
	 *
	 * @return Iconic_WQV_Core_Settings
	 */
	...
	public static function init() {
		...
		self::$settings_framework = new Iconic_WQV_Settings_Framework( self::$args['settings_path'], self::$args['option_group'] );
	...
	public static function add_settings_page() {
		if ( ! defined( 'ICONIC_DISABLE_BRAND' ) && ! defined( 'ICONIC_WQV_DISABLE_BRAND' ) ) {
	...
	public static function hide_notices() {
		$hide_notice = filter_input( INPUT_GET, 'jckqv-hide-notice' );
	...
	public static function output_getting_started_links() {
		...
		?>
		<h3><?php esc_html_e( 'Getting Started', 'jckqv' ); ?></h3>

		<ol>
			<?php foreach ( $links as $link ) { ?>
				<li>
					<a href="<?php echo esc_url( self::get_docs_url() . $link['href'] ); ?>?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckqv&utm_content=getting-started-links" target="_blank"><?php echo esc_html( $link['title'] ); ?></a>
				</li>
			<?php } ?>
		</ol>
		<?php
	...
	public static function setup_dashboard( $settings ) {
		...
		$settings['tabs'][] = array(
			'id'    => 'dashboard',
			'title' => esc_html__( 'Dashboard', 'jckqv' ),
		);

		if ( current_user_can( 'manage_options' ) && ! defined( 'ICONIC_DISABLE_DASH' ) && ! defined( 'ICONIC_WQV_DISABLE_DASH' ) ) {
			$settings['sections']['licence'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'general',
				'section_title'       => esc_html__( 'License &amp; Account Settings', 'jckqv' ),
				'section_description' => '',
				'section_order'       => 10,
				'fields'              => array(
					array(
						'id'       => 'licence',
						'title'    => esc_html__( 'License &amp; Billing', 'jckqv' ),
						'subtitle' => esc_html__( 'Activate or sync your license, cancel your subscription, print invoices, and manage your account information.', 'jckqv' ),
						'type'     => 'custom',
						'output'   => Iconic_WQV_Core_Licence::admin_account_link(),
					),
					array(
						'id'       => 'account',
						'title'    => esc_html__( 'Your Account', 'jckqv' ),
						'subtitle' => esc_html__( 'Manage all of your Iconic plugins, supscriptions, renewals, and more.', 'jckqv' ),
						'type'     => 'custom',
			...
			$settings['sections']['support'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'support',
				'section_title'       => esc_html__( 'Support', 'jckqv' ),
				'section_description' => '',
				'section_order'       => 30,
				'fields'              => array(
					array(
						'id'       => 'support',
						'title'    => esc_html__( 'Support', 'jckqv' ),
						'subtitle' => esc_html__( 'Get premium support with a valid license.', 'jckqv' ),
						'type'     => 'custom',
						'output'   => self::support_link(),
					),
					array(
						'id'       => 'documentation',
						'title'    => esc_html__( 'Documentation', 'jckqv' ),
						'subtitle' => esc_html__( 'Read the plugin documentation.', 'jckqv' ),
						'type'     => 'custom',
	...
	public static function add_sidebar() {
		...
		$cross_sells  = Iconic_WQV_Core_Cross_Sells::get_selected_plugins();
		$api_sidebars = Iconic_WQV_Core_Cross_Sells::get_api_sidebars();
		...
		<div class="iconic-settings-sidebar">
			...
			foreach ( $api_sidebars as $sidebar ) {
				if ( ! empty( $sidebar['content']['rendered'] ) ) {
					...
					<div class="iconic-settings-sidebar__widget iconic-settings-sidebar__widget--blocks">
						<?php echo apply_filters( 'the_content', $sidebar['content']['rendered'] ); ?>
					</div>
					<?php
				}
			}
			...
			if ( ! empty( $cross_sells ) ) {
				...
				<div class="iconic-settings-sidebar__widget iconic-settings-sidebar__widget--works-well">
					<h3><?php esc_html_e( 'Works well with...', 'jckqv' ); ?></h3>
					...
					<?php foreach ( $cross_sells as $cross_sell ) { ?>
						<div class="iconic-product">
							...
							<div class="iconic-product__content">
								<h4 class="iconic-product__title"><a target="_blank" href="<?php echo esc_url( $cross_sell['link'] ); ?>?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckqv&utm_content=cross-sell" target="_blank"><?php echo wp_kses_post( $cross_sell['title']['rendered'] ); ?></a></h4>
									...
									<div class="iconic-product__buttons">
										<p>
											<a href="https://checkout.freemius.com/mode/dialog/plugin/<?php echo esc_attr( $cross_sell['product']['freemius']['plugin_id'] ); ?>/plan/<?php echo esc_attr( $cross_sell['product']['freemius']['plan_id'] ); ?>/" class="button iconic-buy-now iconic-button iconic-button--small" data-plugin-id="<?php echo esc_attr( $cross_sell['product']['freemius']['plugin_id'] ); ?>" data-plan-id="<?php echo esc_attr( $cross_sell['product']['freemius']['plan_id'] ); ?>" data-public-key="<?php echo esc_attr( $cross_sell['product']['freemius']['public_key'] ); ?>" data-type="premium" data-title="<?php echo esc_attr( $cross_sell['title']['rendered'] ); ?>">
												<?php esc_html_e( 'Buy Plugin', 'jckqv' ); ?>
											</a>
										</p>
										<p><?php esc_html_e( '30 Day Money-Back Guarantee', 'jckqv' ); ?></p>
									</div>
								<?php } ?>
							</div>
						</div>
	...
	public static function support_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/support/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckqv&utm_content=support-link', esc_html__( 'Submit Ticket', 'jckqv' ) );
	}
	...
	public static function documentation_link() {
		return sprintf( '<a href="%s?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckqv&utm_content=documentation-link" class="button button-secondary" target="_blank">%s</a>', self::get_docs_url( 'collection' ), esc_html__( 'Read Documentation', 'jckqv' ) );
	}
	...
	public static function account_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/account/?utm_source=Iconic&utm_medium=Plugin&utm_campaign=jckqv&utm_content=account-link', esc_html__( 'Manage Your Account', 'jckqv' ) );
	}
	...
	public static function plugin_action_links( $links ) {
		...
		$action_links = array(
			'settings' => '<a href="' . admin_url( $settings_path ) . '" aria-label="' . esc_attr__( 'View settings', 'jckqv' ) . '">' . esc_html__( 'Settings', 'jckqv' ) . '</a>',
		);
